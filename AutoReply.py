...
"""
    ...
    #天气 <城市名称>：获取指定城市的天气信息，基于wttr.in的API
    #新闻 <可选日期，格式<四位数年份>-<两位数月份>-<两位数天份>（阿拉伯数字）>：60秒看完天下新闻。默认返回当日新闻
    #活字印刷 <文字>：文本转电棍语音
    以#开头的指令无需@机器人也能生效
"""

# 采用的QQ消息框架：LLBot 8.1.8
# 采用的API通信协议：OneBot
# 框架原地址：https://github.com/LLOneBot/LuckyLilliaBot（本机器人也可以配合其他支持OneBot协议的消息框架使用）
...
def handleCommand(event: dict, mtype: str, text: str):
    ...
    if text.startswith("#功能"):
        sendText(targetType, targetId, 
            "1、#点歌 <关键字>\n2、#点赞\n3、#随机美图\n4、#天气 <城市名称>\n"
            "5、#新闻 <可选日期，格式<四位数年份>-<两位数月份>-<两位数天份>（阿拉伯数字）>，默认返回当日新闻\n6、#活字印刷 <文字>")
        return
